...
function buildContextEvidencePrompt(kind, result = {}) {
  ...
  const summaryLines = [];
  const contextPromptReferenceContext = formatPromptReferenceContinuation(evidence.lastPrompt || state.lastPrompt || input?.value || "", { title: "上下文证据关联 @file 引用" });
  const contextDebugTargetContext = formatDebugTargetContinuation(evidence.debugTarget || evidence.diagnostics?.debugTarget || state.lastDebugDiagnostics?.debugTarget || null, { title: "上下文证据关联调试目标" });
  const contextBrowserTriageContext = formatBrowserTriageContinuation(evidence.browserTriage || evidence.diagnostics?.browserTriage || state.lastDebugDiagnostics?.browserTriage || null, { title: "上下文证据关联浏览器异常分诊" });
  ...
  return [
    ...
    "",
    ...summaryLines,
    contextPromptReferenceContext ? `文件引用边界：\n${contextPromptReferenceContext}` : "",
    contextDebugTargetContext ? `当前调试目标：\n${contextDebugTargetContext}` : "",
    contextBrowserTriageContext ? `页面调试线索：\n${contextBrowserTriageContext}` : "",
    "",
    `${title} JSON：`,
...
function buildModelEvidencePrompt(kind, result = {}) {
  ...
  const modelPromptReferenceContext = formatPromptReferenceContinuation(evidence.lastPrompt || state.lastPrompt || input?.value || "", { title: "模型证据关联 @file 引用" });
  const modelDebugTargetContext = formatDebugTargetContinuation(evidence.debugTarget || evidence.diagnostics?.debugTarget || state.lastDebugDiagnostics?.debugTarget || null, { title: "模型证据关联调试目标" });
  const modelBrowserTriageContext = formatBrowserTriageContinuation(evidence.browserTriage || evidence.diagnostics?.browserTriage || state.lastDebugDiagnostics?.browserTriage || null, { title: "模型证据关联浏览器异常分诊" });
  const warnings = [
  ...
  return [
    ...
    "",
    modelPromptReferenceContext ? "文件引用边界：" : "",
    modelPromptReferenceContext,
    modelDebugTargetContext ? "当前调试目标：" : "",
    modelDebugTargetContext,
    modelBrowserTriageContext ? "页面调试线索：" : "",
    modelBrowserTriageContext,
    "",
    "模型证据摘要：",
...
function buildCommandSourceContextPrompt(commandText, run = null, contexts = []) {
  ...
  const commandPromptReferenceContext = formatPromptReferenceContinuation(state.lastPrompt || input?.value || "", { title: "命令源码关联 @file 引用" });
  const commandDebugTargetContext = formatDebugTargetContinuation(run?.result?.diagnostics?.debugTarget || state.lastDebugDiagnostics?.debugTarget || null, { title: "命令源码关联调试目标" });
  const commandBrowserTriageContext = formatBrowserTriageContinuation(run?.result?.diagnostics?.browserTriage || state.lastDebugDiagnostics?.browserTriage || null, { title: "命令源码关联浏览器异常分诊" });
  return [
    "请基于失败命令的源码定位上下文继续修复当前项目。",
    "",
    command ? `失败命令：\n$ ${command}` : "",
    run?.result?.failureAnalysis ? `失败分类：\n${formatCommandFailureAnalysis(run.result.failureAnalysis)}` : "",
    commandPromptReferenceContext ? `文件引用边界：\n${commandPromptReferenceContext}` : "",
    commandDebugTargetContext ? `当前调试目标：\n${commandDebugTargetContext}` : "",
    commandBrowserTriageContext ? `页面调试线索：\n${commandBrowserTriageContext}` : "",
  ...
  try {
    ...
    const draft = await api("/api/source-context-repair-draft", {
      ...
      body: JSON.stringify({
        ...
        debugTarget: state.lastDebugDiagnostics?.debugTarget || null,
        browserTriage: state.lastDebugDiagnostics?.browserTriage || null,
        lastPrompt: state.lastPrompt || input?.value || "",
...
function buildCommandTranscript(commandText, run = null, { title = "请基于这次命令输出继续排查并修复问题。" } = {}) {
  ...
  const commandPromptReferenceContext = formatPromptReferenceContinuation(state.lastPrompt || input?.value || "", { title: "命令记录关联 @file 引用" });
  const commandDebugTargetContext = formatDebugTargetContinuation(diagnostics?.debugTarget || state.lastDebugDiagnostics?.debugTarget || null, { title: "命令记录关联调试目标" });
  const commandBrowserTriageContext = formatBrowserTriageContinuation(diagnostics?.browserTriage || state.lastDebugDiagnostics?.browserTriage || null, { title: "命令记录关联浏览器异常分诊" });
  return [
    title,
    "",
    "命令：",
...
function buildCommandVerificationPrompt(commandText, run = null, { diagnostics = null } = {}) {
  ...
  const sourceLocations = commandSourceLocations(run);
  const commandPromptReferenceContext = formatPromptReferenceContinuation(state.lastPrompt || input?.value || "", { title: "失败命令关联 @file 引用" });
  const commandDebugTargetContext = formatDebugTargetContinuation(attachedDiagnostics?.debugTarget || state.lastDebugDiagnostics?.debugTarget || null, { title: "失败命令关联调试目标" });
  const commandBrowserTriageContext = formatBrowserTriageContinuation(attachedDiagnostics?.browserTriage || state.lastDebugDiagnostics?.browserTriage || null, { title: "失败命令关联浏览器异常分诊" });
  const files = [...new Map(
...
function buildCommandBatchPromptContext(commands = [], { title = "命令组" } = {}) {
  ...
  const batchPromptReferenceContext = formatPromptReferenceContinuation(state.lastPrompt || input?.value || "", { title: "批量命令关联 @file 引用" });
  const batchDebugTargetContext = formatDebugTargetContinuation(state.lastDebugDiagnostics?.debugTarget || null, { title: "批量命令关联调试目标" });
  const batchBrowserTriageContext = formatBrowserTriageContinuation(state.lastDebugDiagnostics?.browserTriage || null, { title: "批量命令关联浏览器异常分诊" });
  ...
  return [
    ...
    referencedFiles.length ? `优先读取相关文件：\n${referencedFiles.map((file) => `@${file}`).join("\n")}` : "",
    batchPromptReferenceContext ? `文件引用边界：\n${batchPromptReferenceContext}` : "",
    batchDebugTargetContext ? `当前调试目标：\n${batchDebugTargetContext}` : "",
    batchBrowserTriageContext ? `页面调试线索：\n${batchBrowserTriageContext}` : "",
...
async function appendFailedCommandSourceContexts(commands = [], { title = "命令组", mode = "evidence" } = {}) {
  ...
  const sourcePromptReferenceContext = formatPromptReferenceContinuation(state.lastPrompt || input?.value || "", { title: "失败源码关联 @file 引用" });
  const sourceDebugTargetContext = formatDebugTargetContinuation(state.lastDebugDiagnostics?.debugTarget || null, { title: "失败源码关联调试目标" });
  const sourceBrowserTriageContext = formatBrowserTriageContinuation(state.lastDebugDiagnostics?.browserTriage || null, { title: "失败源码关联浏览器异常分诊" });
  const context = [
    "请基于这组失败命令的源码上下文继续修复当前项目。",
    "",
    `命令组：${title}`,
    `失败源码定位数量：${items.length}`,
    sourcePromptReferenceContext ? `文件引用边界：\n${sourcePromptReferenceContext}` : "",
    sourceDebugTargetContext ? `当前调试目标：\n${sourceDebugTargetContext}` : "",
    sourceBrowserTriageContext ? `页面调试线索：\n${sourceBrowserTriageContext}` : "",
    "",
    chunks.join("\n\n---\n\n"),
    "",
    verificationCommands.length ? `修复后验证命令：\n${verificationCommands.map((item) => `- ${item.command}${item.reason ? `：${item.reason}` : ""}`).join("\n")}` : "",
...
function buildCatalogEvidenceContext(kind, item = {}) {
  ...
  const catalogPromptReferenceContext = formatPromptReferenceContinuation(item.prompt || state.lastPrompt || input?.value || "", { title: "目录证据关联 @file 引用" });
  const catalogDebugTargetContext = formatDebugTargetContinuation(item.debugTarget || item.diagnostics?.debugTarget || state.lastDebugDiagnostics?.debugTarget || null, { title: "目录证据关联调试目标" });
  const catalogBrowserTriageContext = formatBrowserTriageContinuation(item.browserTriage || item.diagnostics?.browserTriage || state.lastDebugDiagnostics?.browserTriage || null, { title: "目录证据关联浏览器异常分诊" });
  ...
  return [
    ...
    catalogPromptReferenceContext ? `文件引用边界：\n${catalogPromptReferenceContext}` : "",
    catalogDebugTargetContext ? `当前调试目标：\n${catalogDebugTargetContext}` : "",
    catalogBrowserTriageContext ? `页面调试线索：\n${catalogBrowserTriageContext}` : "",
    "",
    "目录证据 JSON：",
...
function buildMcpResourceEvidenceContext(detail = {}) {
  ...
  const mcpResourcePromptReferenceContext = formatPromptReferenceContinuation(detail.prompt || state.lastPrompt || input?.value || "", { title: "MCP 资源证据关联 @file 引用" });
  const mcpResourceDebugTargetContext = formatDebugTargetContinuation(detail.debugTarget || detail.diagnostics?.debugTarget || state.lastDebugDiagnostics?.debugTarget || null, { title: "MCP 资源证据关联调试目标" });
  const mcpResourceBrowserTriageContext = formatBrowserTriageContinuation(detail.browserTriage || detail.diagnostics?.browserTriage || state.lastDebugDiagnostics?.browserTriage || null, { title: "MCP 资源证据关联浏览器异常分诊" });
  ...
  return [
    ...
    mcpResourcePromptReferenceContext ? `文件引用边界：\n${mcpResourcePromptReferenceContext}` : "",
    mcpResourceDebugTargetContext ? `当前调试目标：\n${mcpResourceDebugTargetContext}` : "",
    mcpResourceBrowserTriageContext ? `页面调试线索：\n${mcpResourceBrowserTriageContext}` : "",
    "",
    "Resource 内容：",
...
function buildAssetEvidenceContext(asset = {}, detail = null) {
  ...
  const assetPromptReferenceContext = formatPromptReferenceContinuation(`@file ${asset.path}\n${state.lastPrompt || input?.value || ""}`, { title: "资产证据关联 @file 引用" });
  const assetDebugTargetContext = formatDebugTargetContinuation(state.lastDebugDiagnostics?.debugTarget || null, { title: "资产证据关联调试目标" });
  const assetBrowserTriageContext = formatBrowserTriageContinuation(state.lastDebugDiagnostics?.browserTriage || null, { title: "资产证据关联浏览器异常分诊" });
  return [
    ...
    assetPromptReferenceContext ? `文件引用边界：\n${assetPromptReferenceContext}` : "",
    assetDebugTargetContext ? `当前调试目标：\n${assetDebugTargetContext}` : "",
    assetBrowserTriageContext ? `页面调试线索：\n${assetBrowserTriageContext}` : "",
    "",
    "资产检查摘要：",
...
function restorePendingProposal(goal) {
  ...
  const sourceDebugContext = proposal.sourceDebugContext || {};
  const restoredDebugTarget = proposal.debugTarget || sourceDebugContext.debugTarget || null;
  const restoredBrowserTriage = proposal.browserTriage || sourceDebugContext.browserTriage || null;
  ...
  if (restoredDebugTarget || restoredBrowserTriage) {
    const previousDiagnostics = state.lastDebugDiagnostics || {};
    const restoredDiagnostics = {
      ...previousDiagnostics,
      status: previousDiagnostics.status || "restored_pending_proposal",
      generatedAt: new Date().toISOString(),
      summary: {
        ...(previousDiagnostics.summary || {}),
        restoredPendingProposal: true
      },
      debugTarget: previousDiagnostics.debugTarget || restoredDebugTarget || null,
      browserTriage: previousDiagnostics.browserTriage || restoredBrowserTriage || null,
      findings: previousDiagnostics.findings || [],
      nextActions: previousDiagnostics.nextActions || [],
      verificationPlan: previousDiagnostics.verificationPlan || {
        status: "restored",
        commands: normalizeCommandItems(proposal.commands || [])
      }
    };
    renderDebugDiagnostics(restoredDiagnostics, { persist: true });
    appendToolCall({
      title: "已恢复待审批调试现场",
      label: "debug",
      state: proposal.id,
      body: [
        restoredDebugTarget ? "待审批提案关联调试目标" : "",
        restoredBrowserTriage ? "待审批提案关联浏览器异常分诊" : "",
        JSON.stringify({ debugTarget: restoredDebugTarget, browserTriage: restoredBrowserTriage }, null, 2).slice(0, 8000)
      ].filter(Boolean).join("\n")
    });
  }
...
function buildProcessEvidenceContext(item = {}, { title = "受管进程证据" } = {}) {
  ...
  const processPromptReferenceContext = formatPromptReferenceContinuation(item.prompt || item.lastPrompt || state.lastPrompt || input?.value || "", { title: "进程证据关联 @file 引用" });
  const processDebugTargetContext = formatDebugTargetContinuation(item.debugTarget || item.diagnostics?.debugTarget || state.lastDebugDiagnostics?.debugTarget || null, { title: "进程证据关联调试目标" });
  const processBrowserTriageContext = formatBrowserTriageContinuation(item.browserTriage || item.diagnostics?.browserTriage || state.lastDebugDiagnostics?.browserTriage || null, { title: "进程证据关联浏览器异常分诊" });
  ...
  return [
    ...
    processPromptReferenceContext ? `文件引用边界：\n${processPromptReferenceContext}` : "",
    processDebugTargetContext ? `当前调试目标：\n${processDebugTargetContext}` : "",
    processBrowserTriageContext ? `页面调试线索：\n${processBrowserTriageContext}` : "",
    "",
    "输出尾部：",
...
function buildBrowserEvidenceContext(result = {}, { title = "浏览器证据", kind = "browser" } = {}) {
  ...
  const triage = browserTraceTriage(result);
  const browserPromptReferenceContext = formatPromptReferenceContinuation(result.prompt || result.lastPrompt || result.debugContext?.prompt || state.lastPrompt || input?.value || "", { title: "浏览器证据关联 @file 引用" });
  const browserDebugTargetContext = formatDebugTargetContinuation(result.debugTarget || result.diagnostics?.debugTarget || state.lastDebugDiagnostics?.debugTarget || null, { title: "浏览器证据关联调试目标" });
  const browserTriageContext = formatBrowserTriageContinuation(result.browserTriage || result.diagnostics?.browserTriage || result.trace?.browserTriage || triage || null, { title: "浏览器证据关联浏览器异常分诊" });
  ...
  return [
    ...
    browserPromptReferenceContext ? `文件引用边界：\n${browserPromptReferenceContext}` : "",
    browserDebugTargetContext ? `当前调试目标：\n${browserDebugTargetContext}` : "",
    browserTriageContext ? `页面调试线索：\n${browserTriageContext}` : "",
    "",
    "浏览器异常分诊：",
...
function buildSemanticEvidenceContext(evidence = {}, { title = "语义证据", kind = "semantic" } = {}) {
  ...
  const semanticPromptReferenceContext = formatPromptReferenceContinuation(evidence.lastPrompt || state.lastPrompt || input?.value || "", { title: "语义证据关联 @file 引用" });
  const semanticDebugTargetContext = formatDebugTargetContinuation(evidence.debugTarget || evidence.diagnostics?.debugTarget || state.lastDebugDiagnostics?.debugTarget || null, { title: "语义证据关联调试目标" });
  const semanticBrowserTriageContext = formatBrowserTriageContinuation(evidence.browserTriage || evidence.diagnostics?.browserTriage || state.lastDebugDiagnostics?.browserTriage || null, { title: "语义证据关联浏览器异常分诊" });
  return [
    ...
    semanticPromptReferenceContext ? `文件引用边界：\n${semanticPromptReferenceContext}` : "",
    semanticDebugTargetContext ? `当前调试目标：\n${semanticDebugTargetContext}` : "",
    semanticBrowserTriageContext ? `页面调试线索：\n${semanticBrowserTriageContext}` : "",
    "",
    "语义证据摘要：",
...
function buildGateEvidenceContext(evidence = {}, { title = "门禁证据", kind = "gate" } = {}) {
  ...
  const gatePromptReferenceContext = formatPromptReferenceContinuation(evidence.lastPrompt || evidence.prompt || state.lastPrompt || input?.value || "", { title: "门禁关联 @file 引用" });
  const gateDebugTargetContext = formatDebugTargetContinuation(evidence.debugTarget || evidence.diagnostics?.debugTarget || state.lastDebugDiagnostics?.debugTarget || null, { title: "门禁关联调试目标" });
  const gateBrowserTriageContext = formatBrowserTriageContinuation(evidence.browserTriage || evidence.diagnostics?.browserTriage || state.lastDebugDiagnostics?.browserTriage || null, { title: "门禁关联浏览器异常分诊" });
  return [
    ...
    gatePromptReferenceContext ? `文件引用边界：\n${gatePromptReferenceContext}` : "",
    gateDebugTargetContext ? `当前调试目标：\n${gateDebugTargetContext}` : "",
    gateBrowserTriageContext ? `页面调试线索：\n${gateBrowserTriageContext}` : "",
    "",
    "门禁证据摘要：",
