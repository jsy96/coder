...
function renderQueue(queue = []) {
  ...
  queue.slice(0, 6).forEach((item) => {
    ...
    row.querySelector("small").textContent = `${item.status || "queued"} · P${item.priority || 0} · retry ${item.retryCount || 0}/${item.retryLimit || 0} · ${item.createdAt?.slice(0, 19) || ""}`;
...
async function refreshFiles() {
  ...
  const snapshotSummary = state.contextSnapshot?.generatedAt
    ...
    : "";
  contextText.textContent = `${data.files.length} 个文本文件，约 ${Math.round(data.contextBytes / 1024)} KB 上下文${repoSummary ? `；${repoSummary}` : ""}${snapshotSummary}`;
}
...
browserVisualBtn?.addEventListener("click", async () => {
  ...
  try {
    const result = await api("/api/browser-visual", {
      method: "POST",
      body: JSON.stringify({ url: targetUrl, threshold: 0, maxMismatchRatio: 0 })
    });
