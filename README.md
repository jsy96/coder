...
网页左侧进程区点击“发现”时，会调用 `/api/run-debug-guide` 生成“运行/调试指南”：同时展示 `start.bat` 手动启动入口、`node server.js` 受管启动命令、当前真实调试 URL 和推荐验证命令，避免端口自动切换后不知道该打开哪个页面或先跑哪个检查。
网页左侧仓库区点击“编码简报”时，会调用 `/api/coding-session-brief`，把当前变更文件、运行/调试入口、最近失败命令、恢复线索和建议验证命令整理进提示词，并把建议验证命令放回命令面板，方便下一轮代理直接接着写代码、定位问题并复查。
...
准备包会写入 `.forge/external-readiness/<id>/readiness.md` 和 `readiness.json`，整理授权清单、本地只读预检命令和每个外部阻塞缺口的下一步；行动包会写入 `.forge/external-authorization/<id>/action.md`、`action.json` 和 `handoff-template.json`，把 blocking/manual 授权项转成可执行清单、手工步骤、可复制回填模板和 follow-up verification 命令；集成准备包会写入 `.forge/integration-readiness/<id>/readiness.md` 和 `readiness.json`，提供 MCP server 配置模板、本地扩展 manifest 模板、trust 审计边界和验证命令。`/api/external-readiness-packages`、`/api/external-readiness-package?id=...`、`/api/external-authorization-actions`、`/api/external-authorization-action-package?id=...`、`/api/integration-readiness-packages` 和 `/api/integration-readiness-package?id=...` 可只读复用这些 artifact。网页左侧能力矩阵的“剩余差距摘要”也提供“工作区健康”“集成准备”“授权状态”“行动包”“生成包”“最近包”和“最近行动”按钮，可直接查看启动/验证入口、Git remote、远端 CLI、MCP、扩展、模型 Key、发布证据和权限边界的只读授权状态，生成准备包/行动包/集成准备包、打开最近 artifact、复制外部授权回填模板、把正文加入提示词，并把本地预检或验证命令放入命令面板，方便后续接入远端 PR/CI、MCP、跨站点浏览器或系统级沙箱时逐项推进。
...
- **远端发布预检与继续包**：新增 `/api/remote-publish-preflight`、`/api/remote-publish-continuation`、`/api/remote-publish-evidence-helper`、`/api/remote-publish-evidence-draft`、`/api/remote-publish-evidence`、`remote_publish_preflight`、`remote_publish_continuation` 和 `remote_publish_evidence_helper`，针对发布包汇总审批状态、Git 远端、CLI 安装/认证、命令风险和阻塞项，并生成 `continuation.md` 与 `external-evidence-template.json`，用于人工执行远端动作后回填执行人、时间、远端 URL、PR/MR 编号、CI 链接、评论链接、输出摘要、回滚方案和后续验证命令；Gitee 发布包会标记 `manualProvider`，生成 `manual:gitee-pr` / `manual:gitee-comment` 步骤和 Gitee 仓库 URL，明确要求人工在 Gitee 完成 PR 或评论后回填证据模板；“回填助手”会生成 `gitee-evidence-helper.md`，把继续包、草稿校验、缺失字段清单、可复制证据 JSON 和本地复查命令放在同一张卡片里，减少手工 PR/CI 后不知道填哪里的问题；空模板会先走 `/api/remote-publish-evidence-draft` 生成 `external-evidence-draft.json` 与 `external-evidence-draft.md`，列出缺失字段和可复制 JSON，不把未完成模板标成正式证据；发布包索引和详情会展示草稿状态、缺失字段、草稿 artifact 路径和可复制的证据 JSON，界面里的“复制证据 JSON”按钮可直接拿去补 PR/CI/评论链接后正式回填；回填完整后 `/api/remote-publish-evidence` 会校验必填证据、生成 `external-evidence.json` 与 `external-evidence-summary.md`，并把 publish/gates/core 复查命令回排到本地门禁链路；包索引会展示回填证据状态、远端 URL、PR/CI/评论链接摘要，合并门禁会新增 `remote-publish-external-evidence` gate，把回填后的远端证据纳入交付判断；这些入口不执行 push、建 PR 或远端评论；预检、继续包、回填助手、回填草稿和回填证据结果可一键加入提示词、引用本地 artifact、排队本地复查命令、生成发布回填提示或直接基于 blockers 启动修复；发布审批、发布包读取、预检、继续包和证据回填请求失败时会保留发布包/审批上下文和请求参数，继续生成可修复证据。
...
- **长任务管理**：新增 `/api/processes`、`/api/process-startup-commands`、`/api/run-debug-guide`、`/api/coding-session-brief`、`/api/local-validation-pack`、`/api/process-health`、`/api/process-search`、`/api/process-history`、`/api/runtime-url` 和 `/api/debug-target`，可按 policy 启动受管开发服务、从 `package.json` 和常见入口只读发现推荐启动命令、识别本地端口、把当前真实运行 URL 持久化到 `.forge/state/runtime-url.json`、独立汇总 HTTP 健康探针、持久化 `.forge/process-logs` 日志 artifact、搜索/查看输出尾部、回放历史进程并停止进程；健康接口、启动命令发现、运行/调试指南和编码会话简报会返回 `runtimeUrl`，也会把 `start.bat` 作为 preferred manual launcher 暴露出来，提醒 Windows 用户可走自动端口避让的本地启动入口，同时保留 `node server.js` / `npm start` 这类可受管启动命令；`/api/run-debug-guide` 会聚合 Project Doctor、工作区就绪度、启动入口、当前调试目标和推荐验证命令，前端“发现”会生成可读指南并自动填入受管命令；`/api/coding-session-brief` 会把工作树变更、最近失败命令、恢复线索、调试目标和验证命令整理成可直接加入提示词的 Coding Session Brief；`/api/local-validation-pack` 会按语法快检、日常安全验证、调试闭环、集成与完整本地验证生成只读命令预设，前端“本地验证包”一键放入命令面板，减少每轮改代码后手动拼命令；前端会自动把真实 URL 填入页面调试输入框，避免端口自动切换后误连旧服务；`/api/debug-target` 会把运行 URL、受管进程探针、诊断摘要、验证命令和下一步动作聚合成“当前调试目标”，前端一键诊断会优先展示这个目标并可直接加入提示词；进程区支持“发现”填入推荐命令，“发现并启动”把推荐命令直接送入受管进程启动路径，也支持“发现并调试”在启动后等待探针 URL，自动串起页面检查和浏览器 Trace；进程列表、启动命令发现、运行/调试指南、编码会话简报、健康探针、日志搜索命中和历史 artifact 可一键加入提示词、直接启动基于进程证据的修复，或把探针 URL 直接送入页面检查 / 浏览器 Trace；受管进程行支持“一键调试”，自动串起进程健康、页面检查和浏览器 Trace，生成可加入提示词、引用 artifact、生成带安全检查和页面复查要求的验证提示、直接启动浏览器验证修复，或直接修复的页面调试证据卡；进程证据和失败证据会继续带上当前 `@file` 命中/缺失边界、当前调试目标和浏览器异常分诊，避免服务失败排查时丢掉正在看的源码或页面；一键调试完成后还会沉淀“启动后页面调试恢复”卡，合并健康探针、页面检查、Trace 异常、浏览器异常分诊、下一步动作和复查命令，可加入提示词、放入命令面板、重跑 Trace 或启动验证修复；启动命令发现、发现并启动、发现并调试、启动/停止进程、读取进程输出、搜索日志、读取历史或健康探针失败时，会把命令、endpoint、请求参数、策略/探针上下文和错误栈转成可加入提示词、重试或直接修复的进程失败证据卡。
- **编码驾驶舱**：新增 `/api/coding-cockpit`、`coding_cockpit` 只读工具和“编码驾驶舱”按钮，把运行/调试指南、Coding Session Brief、本地验证包、当前调试目标、Git 变更文件和最近失败命令聚合成一个继续编码提示；一键加入提示词后同时把推荐验证命令放入命令面板，适合每轮修改后快速回到“读变更 -> 调试目标 -> 跑验证 -> 继续修复”的闭环。
- **调试验证分层**：`node server.js --api-smoke-section=debug` 现在保持为日常快速调试门禁，只覆盖核心、运行时和门禁链路；页面检查、Trace、截图、DOM 与视觉能力继续由 `browser` 分段覆盖；需要一次性跑完整调试组合时使用 `node server.js --api-smoke-section=debug-full` 或 `npm run api-smoke:debug-full`，避免常规失败修复路径被浏览器全量 smoke 拖到超时。
...
- **扩展目录**：新增 `/api/extensions` 和侧栏“扩展目录”，扫描 `.forge/extensions/{skills,plugins}` 和可入库的 `extensions/{skills,plugins}` 下的本地 manifest；项目自带 `extensions/plugins/local-readonly-helper/manifest.json`，声明 workspace readiness、Project Doctor、run/debug guide、coding session brief 和 repo map 只读桥接工具。扩展行可查看详情、加入提示词、生成“准备清单”，或生成工具调用“审批示例”，不会绕过审批直接执行扩展工具。
